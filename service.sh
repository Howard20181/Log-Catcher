...
if [ ! -f "$FILE" ]; then
  ...
  BOOT_LOG_TIME=$(gettime "$LOG_PATH/boot.log")
  mv "$LOG_PATH/boot.log" "$LOG_PATH/boot-$BOOT_LOG_TIME.log"
  mv "$LOG_PATH/kernel.log" "$LOG_PATH/kernel-$(gettime "$LOG_PATH/kernel.log").log"
  ...
  for f in "$LOG_PATH"/old/boot-*.log; do
      [ -f "$f" ] || continue
      [ -z "$BOOT_LOG_TIME" ] && BOOT_LOG_TIME=$(gettime "$f") && break
  done
